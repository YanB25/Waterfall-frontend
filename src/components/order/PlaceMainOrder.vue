  ...
  >
    ...
    <el-form-item label="Deadline" prop="_deadline">
      <div>
        ...
        <el-date-picker v-model="ruleForm._deadline" type="date" placeholder="Choose a date"></el-date-picker>
      </div>
    </el-form-item>
...
export default class PlaceMainOrder extends Vue {
  ...
  checkAscii(rule: object, value: string, callback: Callback) {
    if (!value || !validator.isAscii(value)) {
      callback(
        new Error(
          "Should not be empty and ONLY letters and numbers and common punctuations are allowed."
        )
      );
      return;
    }
  ...
  checkNothing(rule: object, value: string, callback: Callback) {
    callback();
  }
    ...
    (this.$refs[formName] as SignUpComponent).validate((valid: any) => {
      if (valid) {
        this.$confirm("你确定要提交吗?", "提交确认", {
          ...
          .then(() => {
            this.ruleForm["deadline"] = this.buildTimeString(
              this.ruleForm._deadline
            );
